...
export default function PageQTD({ QD, B }) {
  ...
  return (
    <div>
      ...
      {/* Charts */}
      <SectionHeader title="QTD vs Q1 Target · Visual" />
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: 14 }}>

        <ChartCard title="ARR vs Q1 Target" sub={`${qtd.month} vs Q1 budget`}
          legend={<><LegendDot color={C.act26} label={qtd.month} /><LegendDot color={C.budLine} label="Target" line dashed /></>}>
          <ResponsiveContainer width="100%" height={200}>
            <ComposedChart data={[
              { name: 'Corp ARR',  act: qtd.corpARR,  tgt: q1Corp  },
              { name: 'Fed ARR',   act: qtd.fedARR,   tgt: q1Fed   },
              { name: 'Total ARR', act: qtd.totalARR, tgt: q1Total },
            ]} margin={{ top: 20, right: 10, left: 0, bottom: 0 }}>
              {GRID}<XAxis dataKey="name" {...XSTYLE} /><YAxis tickFormatter={yFmt$} {...YSTYLE} />
              <Tooltip {...TOOLTIP_STYLE} formatter={v => f$(v)} />
              <Bar dataKey="act" name={qtd.month} fill={C.act26} radius={[3, 3, 0, 0]}>
                <LabelList dataKey="act" position="top" formatter={v => f$(v)} style={{ fill: C.txt3, fontSize: 9 }} />
              </Bar>
              <Line dataKey="tgt" name="Q1 Target" stroke={C.budLine} strokeDasharray="5 4" strokeWidth={2} dot={{ fill: C.budLine, r: 4 }} />
            </ComposedChart>
          </ResponsiveContainer>
        </ChartCard>

        <ChartCard title="Revenue & OpEx vs Q1 Target" sub={`${qtd.month} vs monthly Q1 budget`}
          legend={<><LegendDot color={C.act26} label={qtd.month} /><LegendDot color={C.budLine} label="Monthly Target" line dashed /></>}>
          <ResponsiveContainer width="100%" height={200}>
            <ComposedChart data={[
              { name: 'Revenue', act: qtd.revenue, tgt: q1Rev / 3 },
              { name: 'OpEx',    act: qtd.opex,    tgt: B.opex[0] / 3 },
            ]} margin={{ top: 20, right: 10, left: 0, bottom: 0 }}>
              {GRID}<XAxis dataKey="name" {...XSTYLE} /><YAxis tickFormatter={yFmt$} {...YSTYLE} />
              <Tooltip {...TOOLTIP_STYLE} formatter={v => f$(v)} />
              <Bar dataKey="act" name={qtd.month} fill={C.act26} radius={[3, 3, 0, 0]}>
              ...
              <Line dataKey="tgt" name="Monthly Target" stroke={C.budLine} strokeDasharray="5 4" strokeWidth={2} dot={{ fill: C.budLine, r: 4 }} />
            </ComposedChart>
          </ResponsiveContainer>
        </ChartCard>

        <ChartCard title="Cash vs Q1 Target" sub="Ending cash balance"
          legend={<><LegendDot color={C.cyn} label={qtd.month} /><LegendDot color={C.budLine} label="Target" line dashed /></>}>
          <ResponsiveContainer width="100%" height={200}>
            <ComposedChart data={[
              { name: 'Ending Cash', act: qtd.cash, tgt: q1Cash },
            ]} margin={{ top: 20, right: 10, left: 0, bottom: 0 }}>
              {GRID}<XAxis dataKey="name" {...XSTYLE} /><YAxis tickFormatter={yFmt$} {...YSTYLE} />
              <Tooltip {...TOOLTIP_STYLE} formatter={v => f$(v)} />
              <Bar dataKey="act" name={qtd.month} fill={C.cyn} radius={[3, 3, 0, 0]}>
                <LabelList dataKey="act" position="top" formatter={v => f$(v)} style={{ fill: C.txt3, fontSize: 9 }} />
              </Bar>
              <Line dataKey="tgt" name="Q1 Target" stroke={C.budLine} strokeDasharray="5 4" strokeWidth={2} dot={{ fill: C.budLine, r: 4 }} />
            </ComposedChart>
        ...
        <ChartCard title="Gross Margin % vs Target" sub={`${qtd.month} vs Q1 budget`}
          legend={<><LegendDot color={C.pur} label={qtd.month} /><LegendDot color={C.budLine} label="Target" line dashed /></>}>
          <ResponsiveContainer width="100%" height={200}>
            <ComposedChart data={[{ name: 'GM%', act: (qtd.gm || 0) * 100, tgt: (q1Gm || 0) * 100 }]}
              margin={{ top: 20, right: 10, left: 0, bottom: 0 }}>
              {GRID}<XAxis dataKey="name" {...XSTYLE} /><YAxis tickFormatter={v => `${v}%`} {...YSTYLE} domain={[75, 95]} />
              <Tooltip {...TOOLTIP_STYLE} formatter={v => `${v.toFixed(1)}%`} />
              <Bar dataKey="act" name={qtd.month} fill={C.pur} radius={[3, 3, 0, 0]}>
                <LabelList dataKey="act" position="top" formatter={v => `${v.toFixed(1)}%`} style={{ fill: C.txt3, fontSize: 9 }} />
              </Bar>
              <Line dataKey="tgt" name="Q1 Target" stroke={C.budLine} strokeDasharray="5 4" strokeWidth={2} dot={{ fill: C.budLine, r: 4 }} />
            </ComposedChart>
          </ResponsiveContainer>
        ...
        <ChartCard title="Corp NRR % vs Target" sub={`${qtd.month} TTM`}
          legend={<><LegendDot color={C.grn} label={qtd.month} /><LegendDot color={C.budLine} label="Target" line dashed /></>}>
          <ResponsiveContainer width="100%" height={200}>
            <ComposedChart data={[{ name: 'NRR%', act: (qtd.nrr || 0) * 100, tgt: (q1Nrr || 0) * 100 }]}
              margin={{ top: 20, right: 10, left: 0, bottom: 0 }}>
              {GRID}<XAxis dataKey="name" {...XSTYLE} /><YAxis tickFormatter={v => `${v}%`} {...YSTYLE} domain={[100, 115]} />
              <Tooltip {...TOOLTIP_STYLE} formatter={v => `${v.toFixed(1)}%`} />
              <Bar dataKey="act" name={qtd.month} fill={C.grn} radius={[3, 3, 0, 0]}>
                <LabelList dataKey="act" position="top" formatter={v => `${v.toFixed(1)}%`} style={{ fill: C.txt3, fontSize: 9 }} />
              </Bar>
              <Line dataKey="tgt" name="Q1 Target" stroke={C.budLine} strokeDasharray="5 4" strokeWidth={2} dot={{ fill: C.budLine, r: 4 }} />
            </ComposedChart>
