...
export default function PageLTO({ QD, B, ltYears, ltForecast, cashOutDate }) {
  ...
  // MDA computations — derived from arrData + ltYears
  const arr25Total = (arrData[0]?.corp || 0) + (arrData[0]?.fed || 0);
  const arr26Total = (arrData[1]?.corp || 0) + (arrData[1]?.fed || 0);
  const arrLast    = arrData[arrData.length - 1];
  const arrLastTot = arrLast ? (arrLast.corp || 0) + (arrLast.fed || 0) : null;
  // CAGR from 2025A to last available year
  const ltYearCount = arrData.length - 1;  // years from 2025 to last year
  const cagr = (arr25Total && arrLastTot && ltYearCount > 1)
    ? Math.pow(arrLastTot / arr25Total, 1 / ltYearCount) - 1
    : null;
  const mdaRev25 = ann(QD.rev, 0);
  const mdaRev26 = ann(QD.rev, 4);
  const revLast = hasLT ? lt.revenue?.[OUT_YEARS[OUT_YEARS.length - 1]] : null;
